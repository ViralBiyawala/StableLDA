...
import random
import operator
from scipy.stats import binom, binomtest as binom_test
import os
import sys
import io
...
class StableLDA():
    ...
    def init_word_cluster(self, embed_method):
        ...
        if embed_method == 'cbow':
            self.w2v_model = Word2Vec(sentences=self.text, vector_size=100, window=3, min_count=5, workers=1, epochs=10, sg=0)
        elif embed_method == 'skip-gram':
            self.w2v_model = Word2Vec(sentences=self.text, vector_size=100, window=3, min_count=5, workers=1, epochs=10, sg=1)
        elif embed_method == 'fasttext':
            self.w2v_model = FastText(sentences=self.text, vector_size=100, window=3, min_count=5, workers=1, epochs=10)
        ...
        self.kmeans = KMeans(n_clusters=self.num_cluster, algorithm='elkan', n_init=10, init='k-means++', random_state=0).fit(self.embeddings)
        #self.kmeans = AgglomerativeClustering(n_clusters=self.num_cluster).fit(self.embeddings)


    def initialize(self):
        '''
        this method uses the topical clusters to guide stable lda, building constraints for dirichlet forest prior
        '''
        ...
        for doc in self.bow: # for each document, examine frequent topics
            ...
            for k, v in topic_usage.items():
                binom_pvalue = binom_test(v, len(doc), topic_pr[k], alternative='greater').pvalue
